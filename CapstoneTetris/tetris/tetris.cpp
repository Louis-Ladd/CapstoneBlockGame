...
void Tetris::BuildUI() {
  ...
  UILabel *game_over_label =
      new UILabel(0, 0, this->ui_manager.GetDefaultFont(3), "Game Over", white,
                  this->application->renderer);
  ...
  continue_button->button->SetOnClickFunction([this](void) {
  ...
  exit_to_main_menu_button->button->SetOnClickFunction([this](void) {
    this->SetGameOverUIElements(false);
  ...
  quit_button->button->SetOnClickFunction(
      [this](void) { this->application->Quit(); });
...
void Tetris::UpdateClearedLines() {
  ...
  for (int i = 0; i < BOARD_HEIGHT; i++) {
    ...
    for (int relative_row = i; relative_row > 0; relative_row--) {
      std::copy(board[relative_row - 1], board[relative_row - 1] + BOARD_WIDTH,
                board[relative_row]);
    }
  }
